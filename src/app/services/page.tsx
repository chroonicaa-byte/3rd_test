...
import { ArrowRight, CheckCircle2, Building2, BookOpen, Mic, Laptop, GraduationCap, Users2 } from 'lucide-react'
export const metadata: Metadata = { title: 'Services – HR Consulting, Training & Career Advisory' }

interface Service {
  name: string; desc: string; price: string; priceNote: string; features: string[]; cta: string; highlight?: boolean
}
interface Category {
  id: string; icon: React.ElementType; label: string; tag: string; tagColor: string; headline: string; subhead: string; services: Service[]
}
...
function ServiceCard({ s }: { s: Service }) {
  return (
    <div className={`rounded-2xl p-7 flex flex-col h-full border transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${s.highlight ? 'border-gold-300 bg-gold-50 ring-2 ring-gold-200' : 'border-border bg-white'}`}>
      {s.highlight && <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-gold-gradient text-brand-900 text-xs font-semibold mb-4 w-fit">★ Most Popular</div>}
      ...
      <Link href="/contact" className={`inline-flex items-center justify-center gap-2 w-full py-3 rounded-xl text-sm font-semibold transition-all ${s.highlight ? 'bg-gold-gradient text-brand-900 shadow-md hover:opacity-90' : 'border-2 border-brand-800 text-brand-900 hover:bg-brand-800 hover:text-white'}`}>
  ...
  return (
    <>
      ...
      <section className="bg-brand-gradient pt-32 pb-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="max-w-2xl">
            <p className="text-white/70 text-sm font-semibold tracking-widest uppercase mb-4">Everything We Offer</p>
            <h1 className="heading-hero mb-6">Services & Pricing</h1>
            <p className="text-white/80 text-xl leading-relaxed">Every service has a clear price. No discovery call required to know what you're getting into.</p>
          </div>
        </div>
      ...
      <div className="sticky top-16 z-30 bg-white border-b border-border shadow-sm">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-1 overflow-x-auto py-3">
            {categories.map((c) => (
              <a key={c.id} href={`#${c.id}`} className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-medium text-muted-foreground hover:text-brand-900 hover:bg-background transition-colors whitespace-nowrap">
      ...
      {categories.map((cat, idx) => (
        <section key={cat.id} id={cat.id} className={`section-pad ${idx % 2 === 0 ? 'bg-background' : 'bg-white'}`}>
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col md:flex-row md:items-end gap-4 mb-12">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-semibold ${cat.tagColor}`}>{cat.tag}</span>
                </div>
                <h2 className="heading-lg text-foreground mb-2">{cat.headline}</h2>
                <p className="text-muted-foreground">{cat.subhead}</p>
              </div>
              <Link href="/contact" className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border-2 border-brand-800 text-brand-900 text-sm font-semibold hover:bg-brand-800 hover:text-white transition-all whitespace-nowrap">
            ...
            <div className={`grid gap-6 ${cat.services.length === 2 ? 'md:grid-cols-2 max-w-3xl' : cat.services.length === 3 ? 'md:grid-cols-3' : 'md:grid-cols-2 lg:grid-cols-4'}`}>
              {cat.services.map((s) => <ServiceCard key={s.name} s={s} />)}
            </div>
          </div>
        </section>
      ))}
